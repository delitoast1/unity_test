    ...
    class SlicesMetadata
    {
        ...
        private void JoinPointsAlongPlane()
        {
            ...
            for (int i = 0; i < _pointsAlongPlane.Count; i += 2)
            {
                ...
                Vector3 normal3 = ComputeNormal(halfway, secondVertex, firstVertex).normalized;

                Vector2 capUV = new Vector2(UnityEngine.Random.value, UnityEngine.Random.value);

                var direction = Vector3.Dot(normal3, _plane.normal);
                if (direction > 0)
                {
                    AddTrianglesNormalAndUvs(MeshSide.Positive, halfway, -normal3, capUV, firstVertex, -normal3, capUV, secondVertex, -normal3, capUV, false, true);
                    AddTrianglesNormalAndUvs(MeshSide.Negative, halfway, normal3, capUV, secondVertex, normal3, capUV, firstVertex, normal3, capUV, false, true);
                }
                else
                {
                    AddTrianglesNormalAndUvs(MeshSide.Positive, halfway, normal3, capUV, secondVertex, normal3, capUV, firstVertex, normal3, capUV, false, true);
                    AddTrianglesNormalAndUvs(MeshSide.Negative, halfway, -normal3, capUV, firstVertex, -normal3, capUV, secondVertex, -normal3, capUV, false, true);
                }
            }
